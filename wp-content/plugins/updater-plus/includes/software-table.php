		...
		*/
		function __construct() {
			parent::__construct( array(
				'singular'  => __( 'item', 'updater-plus' ),
				'plural'    => __( 'items', 'updater-plus' )
			) );
		}
		/**
		...
		*/
		...
		function get_columns() {
			$columns = array(
				...
				'title'			=> __( 'Title', 'updater-plus' ),
				'version'   	=> __( 'Version', 'updater-plus' ),
				'auto_update' 	=> __( 'Auto Update', 'updater-plus' ),
				'category'  	=> __( 'Category', 'updater-plus' )
			);
			return $columns;
		}
		/**
		...
		function get_bulk_actions() {
			$actions = array();
			$actions['update'] = __( 'Update Now', 'updater-plus' );
			return $actions;
		}
		/**
			...
			switch ( $column_name ) {
				case 'version':
					...
					if ( ! empty( $item['new_version'] ) && $item['version'] != $item['new_version'] ) {
						$return .= '<div>' . __( 'New version is available.', 'updater-plus' ) . ' ';
						...
						if ( $item['type'] != 'core' ) {
							...
							$return .= ' ' . __( 'or', 'updater-plus' ) . ' <a class="pdtr-update-now" href="' . wp_nonce_url( self_admin_url( 'admin.php?page=updater-plus&pdtr_tab_action=update' . $link_part ), 'updater-update' . $item['wp_key'] ) . '">' . __( 'update now', 'updater-plus' ) . '.</a>';
						} else {
							$return .= '<a href="' . wp_nonce_url( self_admin_url( 'admin.php?page=updater-plus&pdtr_tab_action=update' . $link_part ), 'updater-update' . $item['wp_key'] ) . '">' . __( 'Update now.', 'updater-plus' ) . '</a>';
						}
						$return .= '</div>';
					}
					return $return;
				case 'category':
					if ( 'plugin' == $item['type'] )
						return __( 'Plugin', 'updater-plus' );
					elseif ( 'theme' == $item['type'] )
						return __( 'Theme', 'updater-plus' );
					else
						return __( 'Core', 'updater-plus' );
				case 'auto_update':
				    if ( 1 == $pdtr_options['mode'] ) {
					    return __( 'Enabled', 'updater-plus' );
                    } else {
				        return __( 'Disabled', 'updater-plus' );
                    }
			...
			if ( ! empty( $plugin_list_for_update ) || ! empty( $theme_list_for_update ) || $core_for_update ) {
				...
				if ( 1 == $pdtr_options["send_mail_after_update"] ) {
					if ( $core_result || ! empty( $plugin_list_for_update ) || ! empty( $theme_list_for_update ) ) {
						echo '<h2>' . __( 'Sending Report...', 'updater-plus' ) . '</h2>';
						...
						if ( $result_mail != true ) { ?>
							<p><?php printf( __( "Sorry, your email could not be sent to %s.", 'updater-plus' ), implode( ",", $emails ) ); ?></p>
						<?php } else { ?>
							<p><?php printf( __( "The email with the update results was sent to %s.", 'updater-plus' ), implode( ",", $emails ) ); ?></p>
						<?php }
					}
				} ?>
				<p><a target="_parent" title="<?php _e( 'Go to the Updater Plus page', 'updater-plus' ); ?>" href="admin.php?page=updater-plus"><?php _e( 'Go back to the Updater Plus page', 'updater-plus' ); ?></a></p>
			<?php }
